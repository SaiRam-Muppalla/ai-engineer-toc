# Stage 5 — LLM Engineering Resources

## 📺 YouTube Content

| Resource | Description |
|----------|-------------|
| [LangChain Crash Course (Patrick Loeber)](https://www.youtube.com/watch?v=LbT1yp6quS8) | Best LangChain intro |
| [RAG from Scratch (LangChain Official)](https://www.youtube.com/watch?v=wd7TZ4w1mSw) | Official RAG tutorial |
| [LLM Agents (Samuel Chan)](https://www.youtube.com/watch?v=AxnL5GtWVNA) | AI agents explained |
| [Andrej Karpathy — Let's build GPT](https://www.youtube.com/watch?v=kCc8FmEb1nY) | Build a GPT from scratch |
| [Fine-Tuning LLMs (Trelis Research)](https://www.youtube.com/c/TrelisResearch) | Practical fine-tuning |
| [AI Jason — LangGraph Tutorials](https://www.youtube.com/@AIJason) | LangGraph and agents |
| [DeepLearningAI YouTube](https://www.youtube.com/@Deeplearningai) | LLM courses, RAG tutorials, prompt engineering |

## 🎓 Free Courses (DeepLearning.AI)

| Course | Link |
|--------|------|
| ChatGPT Prompt Engineering | https://www.deeplearning.ai/short-courses/chatgpt-prompt-engineering-for-developers/ |
| LangChain for LLM App Dev | https://www.deeplearning.ai/short-courses/langchain-for-llm-application-development/ |
| Building RAG Agents with LlamaIndex | https://www.deeplearning.ai/short-courses/building-agentic-rag-with-llamaindex/ |
| LLMOps | https://www.deeplearning.ai/short-courses/llmops/ |
| Fine-Tuning Large Language Models | https://www.deeplearning.ai/short-courses/finetuning-large-language-models/ |
| Multi-Agent Systems with CrewAI | https://www.deeplearning.ai/short-courses/multi-ai-agent-systems-with-crewai/ |

## 📘 Key Guides & Documentation

| Resource | Link |
|----------|------|
| Prompt Engineering Guide | https://www.promptingguide.ai/ |
| LangChain Documentation | https://python.langchain.com/docs/ |
| LlamaIndex Documentation | https://docs.llamaindex.ai/ |
| LangGraph Documentation | https://langchain-ai.github.io/langgraph/ |
| HuggingFace PEFT | https://huggingface.co/docs/peft |
| OpenAI API Docs | https://platform.openai.com/docs/ |

## 🤖 Open Source Models to Explore

| Model | Use Case | Access |
|-------|---------|--------|
| Llama 3 (Meta) | General purpose | https://huggingface.co/meta-llama |
| Mistral 7B | Efficient, fast | https://huggingface.co/mistralai |
| CodeLlama | Code generation | https://huggingface.co/codellama |
| Phi-3 (Microsoft) | Small but powerful | https://huggingface.co/microsoft |
| Gemma (Google) | General purpose | https://huggingface.co/google |

## 🔧 Tools & APIs

| Tool | Purpose | Link |
|------|---------|------|
| Ollama | Run LLMs locally | https://ollama.ai/ |
| Groq | Ultra-fast inference | https://console.groq.com/ |
| Together AI | Open model hosting | https://www.together.ai/ |
| Replicate | GPU inference | https://replicate.com/ |
| Cohere | Embeddings + LLMs | https://cohere.com/ |
| Pinecone | Vector DB | https://www.pinecone.io/ |
| Chroma | Open vector DB | https://www.trychroma.com/ |

## 📚 Best Books

| Book | Author(s) |
|------|-----------|
| Build a Large Language Model From Scratch | Sebastian Raschka |
| Hands-On Large Language Models | Jay Alammar, Maarten Grootendorst |
| Designing Machine Learning Systems | Chip Huyen |
| Natural Language Processing with Transformers | Lewis Tunstall et al. |

## 🌐 Best Open Source Resources

| Resource | Link |
|----------|------|
| HuggingFace Transformers | https://github.com/huggingface/transformers |
| LangChain | https://github.com/langchain-ai/langchain |
| LlamaIndex | https://github.com/run-llama/llama_index |
| Awesome LLM | https://github.com/Hannibal046/Awesome-LLM |
| OpenAI Cookbook | https://github.com/openai/openai-cookbook |

## 🎓 Best Free PDFs / Courses

| Course | Link |
|--------|------|
| Stanford CS224n (NLP with Deep Learning) | https://cs224n.stanford.edu/ |
| Stanford CS324 (Large Language Models) | https://stanford-cs324.github.io/winter2022/ |
| Princeton COS 597G (Understanding Large Language Models) | https://www.cs.princeton.edu/courses/archive/fall22/cos597G/ |
